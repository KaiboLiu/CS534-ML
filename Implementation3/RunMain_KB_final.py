...
def run_part(data_train,data_test,k_max=1,saveDir="./iris-Result/",rate_subset=1,L=1):
    t0 = float(time.clock())
    f = [0,1,2,3]
    ...
    for k in range(1,k_max+1):
        ...
        for l in range(L): #creat L trees for one k
            ...
            if saveDir[-2] == '2':   #if it's problem2, choose random features
                feature_pool = random.sample(f, 2)  #choose 2/4 features out of 4
                feature_pool.sort()
            else: 
                feature_pool = f
            if rate_subset < 1:     ##if it's problem2, and subset n<N, choose random subset
                subset = []
                for i in range(n_subset):
                    subset.append(data_train[int(random.uniform(0, len(data_train)-1)),:])
                    tree = buildtree(np.array(subset),k,feature_pool)
            else:
                tree = buildtree(data_train,k,feature_pool)
    ...
    error_train[:] = 1/float(len(data_train))*error_train
    error_test[:] = 1/float(len(data_test))*error_test
    t1 = float(time.clock())
    ...
    if saveDir[-2] == '1':
        ...
        plt.grid(True)
        plt.savefig(saveDir+"error vs k=0-%d" %(k_max))
    elif saveDir[-2] == '2':
...
def RunMain():
    ...
    for l in Loop_list:
        ...
        for iteration in range(10): #10 random runs
            error_train,error_test =run_part(data_train,data_test,k_max,saveDir+'Part2_',rate_subset,l)
        ...
        plt.legend(loc='upper right')
        plt.grid(True)
    plt.savefig(saveDir+'Part2_L_trees_error vs k=%d' %(k_max))
